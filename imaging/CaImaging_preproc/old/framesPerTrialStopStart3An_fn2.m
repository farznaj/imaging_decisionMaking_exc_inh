...
% You used to used it for session with the state trial_start_rot_scope, but
% then you incorporated this function into framesPerTrialStopStart3An_fn,
% so now you have a single function for all sessions. so no need for this
% function!
%
...
if ~isfield(all_data(1).parsedEvents.states, 'trial_start_rot_scope')
    error('Use script framesPerTrialStopStart3An_fn. This session does not have the state trial_start_rot_scope!')
end
...
% trial code.
startRotScope_trialCode1_dur = NaN(1, length(all_data)); % length(codeTimes)
for tr = 1:length(all_data) % length(codeTimes)
...
%     error('You need to check codes below.')
  ...
  diffs = (startOffSamples + 1) - codeTimes(1:length(startOffSamples)); % interval between the end of trialStart and the beginning of trial code.
  diffAlign = abs(startRotScope_trialCode1_dur + diffs);  % I don't really expect the 2 values to be >1 different. 
  
  while length(trialNumbers) > nTrials
    firstBad = find(diffAlign > 1, 1);
%     figure; plot(startRotScope_trialCode1_dur + diffs)
%     firstBad = find(abs(diffs) > 10, 1);

    if diffAlign(firstBad) < 10 && diffAlign(firstBad) > 1        
         error('You need to check why this is happening!')    
         % I don't expect the 2 values really to be >1 different.
         % Matt seems to take care of it but you need to check if it
         % works for you.
    end
