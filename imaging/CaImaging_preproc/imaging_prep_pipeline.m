...
r = repmat('%d-', 1, length(mdfFileNumber)); r(end) = []; 
outName = sprintf(['%s-%s-',r], mouse, imagingFolder, mdfFileNumber)
...
P.motionCorrDone = 0; % if 0, the below matters.
    ...
    % if 0
    P.regFrameNums = [1:200]; % {2}; % {2}; % noMotionTr
...
params = writeCaProcessParams(outName, mouse, imagingFolder, mdfFileNumber, P);
...
[cs_frtrs, Nnan_nanBeg_nanEnd] = update_tempcomps_multitrs_setvars...
    (mouse, imagingFolder, mdfFileNumber); % , p);
