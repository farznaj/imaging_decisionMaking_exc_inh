...
%% Find trials for which trialStart was not recorded), and gives you clue on how to fix the mismatch in size of nTrials and trialNumbers.
...
%% Cross-check start TTLs and trial numbers (since start_rotary_scop will be always 

if length(trialNumbers) > nTrials
    warning('framesPerTrialStopStart3An:missingStarts', 'Missing %d trial starts', ...
...
diffs = (startOffSamples + 1) - codeTimes; % this is the gap between trialStart offset and trialCode onset (when they were both low). If animal waited in start_rotary_scope for dur_state, we expect this gap to be 0 (since trialCode is sent in the state immediately following start_rotary_scope). If animal went into start_rotary_scope2 and then trialCode state, then this gap should be dur_state (which is the duration of start_rotray_scope2).
...
badAlignTrStartCode = ~ismember(diffs,[0, -dur_state, -1, -dur_state-1]); % -1 needs check: matt said he allows 1ms for slightly unequal rise and fall times of the different ttl lines. 
