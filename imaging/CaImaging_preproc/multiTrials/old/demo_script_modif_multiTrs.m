...
[Am,Cm,K_m,merged_ROIs,P,Sm] = merge_components_multiTrs(Yr,A,b,C,f,P,S,options);
...
% u need to send ff (index of removed comps) out from update_space to
% remove them from C


% Remove iti values from Cm and f to make it same size as Yr
Cm0 = Cm;
Cm_nonan = Cm;
f_nonan = f;
a = cell2mat(arrayfun(@(x,y)x:y, nanBeg, nanEnd, 'uniformoutput', 0));
...
[A2,b2,Cm] = update_spatial_components(Yr,Cm_nonan,f_nonan,Am,P,options); 
...
%     if concatTempUpdate % do on concat movie:
        [C2,f2,P,S2] = update_temporal_components_multiTrs_itiNaN(Yr,A2,b2,Cm,f,P,options, cs_frtrs);
