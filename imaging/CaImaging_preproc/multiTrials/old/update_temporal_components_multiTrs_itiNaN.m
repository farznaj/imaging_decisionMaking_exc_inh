function [C,f,P,S,YrA] = update_temporal_components_multiTrs_itiNaN(Y,A,b,Cin,fin,P,options, cs_frtrs, nFrames_iti)
...
for itr = 1:length(cs_frtrs)-1
    ...
    y = [y; YrA(frs,:)];
    cintmp = [cintmp, Cin(:,frs)];
%     fintmp = [fintmp, fin(frs)];
    if itr < length(cs_frtrs)-1
        y = [y; NaN(Nnan(itr), size(YrA,2))];
        cintmp = [cintmp, NaN(size(YrA,2), Nnan(itr))];
%         fintmp = [fintmp, NaN(1, Nnan(itr))];
    end
end
T = size(y,1);
YrA = y;
...
if options.temporal_parallel
    for iter = 1:ITER
        ...
        for jo = 1:length(O)
            ...
            % FN added 
            % put Ytemp for each trial in a cell.
            Ytrs = cell(1, length(cs_frtrs_nan)-1);
            for itr = 1:length(cs_frtrs_nan)-1
%                 frs = cs_frtrs_nan(itr)+1 : cs_frtrs_nan(itr+1);
                frs = cs_frtrs_nan(itr)+1 : cs_frtrs_nan(itr) + n_frtrs(itr);
                Ytrs{itr} = Ytemp(frs,:); % Ytemp(frs,jj);
            end
            %{
            y = [];
            Nnan = nFrames_iti(:);      % lengths of inter-trial intervals that have been removed
            for itr = 1:length(Ytrs)
                y = [y; Ytrs{itr}];
                if itr < length(Ytrs)
                    y = [y; NaN(Nnan(itr), size(Ytrs{1},2))];
                end
            end
            T = size(y,1);
            Ytemp = y;
...
%                             gtrs = estimate_g_multi_trials(Ytrsnow); % ,p,lags,fudge_factor)
                            gtrs = estimate_g_multi_trials(Ytrsnow,[],[],1); % use fudge_factor 1, needs assessment.
