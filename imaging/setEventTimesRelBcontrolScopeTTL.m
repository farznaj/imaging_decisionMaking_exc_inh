...
%% All times (ms) are relative to when bcontrol sent scope ttl. (this makes more sense than timeStartScan_abs, since alldata.frameTimes is also relative to bcontrol ttl time.)
...
for tr = 1:length(alldata)
    ...
    if ~ismember(tr, trs2rmv)
        ...
        if isfield(alldata(tr).parsedEvents.states, 'trial_start_rot_scope')
            timeScopeTTLsent_abs = alldata(tr).parsedEvents.states.trial_start_rot_scope(1,1) * 1000;
        else
        ...
        timeNoCentLickOnset(tr) = last_start_rotary_scope(f,1) * 1000 - timeScopeTTLsent_abs; % onset of the epoch during which mouse did not lick center (and side)
        timeNoCentLickOffset(tr) = last_start_rotary_scope(f,2) * 1000 - timeScopeTTLsent_abs; % same as trialCode onset. % offset of the epoch during which mouse did not lick center (and side)
        ...
        timeInitTone{tr} = [alldata(tr).parsedEvents.states.wait_for_initiation(:,1);...
            alldata(tr).parsedEvents.states.wait_for_initiation2(:,1)] * 1000 - timeScopeTTLsent_abs;
        ...
        if ~ismember(outcomes(tr), -3) % dont analyze wrong start.
            time1stCenterLick(tr) = max([alldata(tr).parsedEvents.states.wait_for_initiation(:,2);...
                alldata(tr).parsedEvents.states.wait_for_initiation2(:,2)]) * 1000 - timeScopeTTLsent_abs;
        end
        ...
        if ~isempty(alldata(tr).parsedEvents.states.wait_stim) && ~ismember(outcomes(tr), -1) % -1 added so early decisions are not analyzed for stim, bc the stim ends at the middle for them. % you commented the line above bc if the mouse licks during stim pre delay it will be counted as early decision (-1) hence outcome is not -3 but still no stimulus was played.
            timeStimOnset(tr) = alldata(tr).parsedEvents.states.wait_stim(1) * 1000 - timeScopeTTLsent_abs;
        end
        ...
        if ~ismember(outcomes(tr), invalid) % dont analyze early decision , wrong start , or no center commit
            timeCommitCL_CR_Gotone(tr) = alldata(tr).parsedEvents.states.center_reward(1) * 1000 - timeScopeTTLsent_abs;
        end
        
        % first 1st side try and 1st correct/ incorrect try, relative to scan start. I am
        ...
        if ~ismember(outcomes(tr), [invalid, noDecision])
            if ~isempty(alldata(tr).parsedEvents.states.correctlick_again_wait)
                time1stCorrectTry(tr) = min(alldata(tr).parsedEvents.states.correctlick_again_wait(:,1)) * 1000 - timeScopeTTLsent_abs;
            end
            
            if ~isempty(alldata(tr).parsedEvents.states.errorlick_again_wait)
                time1stIncorrectTry(tr) = min(alldata(tr).parsedEvents.states.errorlick_again_wait(:,1)) * 1000 - timeScopeTTLsent_abs;
            end
            
            time1stSideTry(tr) = min([alldata(tr).parsedEvents.states.correctlick_again_wait(:,1);...
                alldata(tr).parsedEvents.states.errorlick_again_wait(:,1)]) * 1000 - timeScopeTTLsent_abs;
        end
        ...
        if ismember(outcomes(tr), 1)
            timeReward(tr) = alldata(tr).parsedEvents.states.reward(1) * 1000 - timeScopeTTLsent_abs;
        end
        ...
        if ~ismember(outcomes(tr), [invalid, noSideCommit])
            if ~isempty(alldata(tr).parsedEvents.states.punish_allowcorrection)
                timeCommitIncorrResp(tr) = alldata(tr).parsedEvents.states.punish_allowcorrection(:,1) * 1000 - timeScopeTTLsent_abs;
            elseif ~isempty(alldata(tr).parsedEvents.states.punish)
                timeCommitIncorrResp(tr) = alldata(tr).parsedEvents.states.punish(:,1) * 1000 - timeScopeTTLsent_abs;
            end
        end
        
        % stop_rotary_scope
        timeStop(tr) = alldata(tr).parsedEvents.states.stop_rotary_scope(1,1) * 1000 - timeScopeTTLsent_abs;
        
        
        % time of all licks : negative values indicate licks that happened
        % before the start of scopeTTL.
        centerLicks{tr} = alldata(tr).parsedEvents.pokes.C(:,1) * 1000 - timeScopeTTLsent_abs;
        leftLicks{tr} = alldata(tr).parsedEvents.pokes.L(:,1) * 1000 - timeScopeTTLsent_abs;
        rightLicks{tr} = alldata(tr).parsedEvents.pokes.R(:,1) * 1000 - timeScopeTTLsent_abs;
        ...
        if ~ismember(outcomes(tr), -3)            
            ...
            timeStimOffset(tr) = stimEndActual - timeScopeTTLsent_abs;            
        end
%         % stimulus offset
%         if ~ismember(outcomes(tr), -3)
%             timeStimOffset(tr) = alldata(tr).parsedEvents.states.wait_stim(2) * 1000 - timeScopeTTLsent_abs;
%         end

        
        % first correct lick (not the 2nd, commit lick) that resulted in a reward.
        if ismember(outcomes(tr), 1)
            ...
            time1stCorrectResponse(tr) = caw_yes * 1000 - timeScopeTTLsent_abs;
            ...
            % if the correct response was followed in shorter than 100ms by
            % a center lick, exclude that trial.
            if max(time1stCorrectResponse(tr) - (cf * 1000 - timeScopeTTLsent_abs)) >= -100
