function [bb,dev,stats] = glmfit_me(x,y,distr,varargin)
...
iter = 0;
iterLim = 100; % 100; % FN changed 100 to 200.
warned = false;
seps = sqrt(eps);
convcrit = 1e-6;
...
while iter <= iterLim
    ...
    % Sum components of deviance to get the total deviance.  % FN copied
%     di = devFun(mu,y);
%     dev_a(iter) = sum(pwts .* di);
...
[varargout{1:nargout}] = internal.stats.insertnan(wasnan,varargin{:});
