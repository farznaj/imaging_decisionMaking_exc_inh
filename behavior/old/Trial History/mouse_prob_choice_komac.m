...
function [bs bf b0] = mouse_prob_choice_komac(mousename,ndays,theDate, fig)
...
modality = [dat.visual_or_auditory];
...
for jj = 2:length(dat)
dat(jj).last_trial_from_same_session = 1 - diff([dat(jj-1:jj).session_count]);
end
...
trial_success_history(2:end) = ([dat(2:end).last_trial_from_same_session] == 1) .* (-1*([dat(1:end-1).resp] == 1 & [dat(1:end-1).correct_choice] == 0) + 1*([dat(1:end-1).resp] == 2 & [dat(1:end-1).correct_choice] == 1));
trial_failure_history(2:end) = ([dat(2:end).last_trial_from_same_session] == 1) .* (-1*([dat(1:end-1).resp] == 1 & [dat(1:end-1).correct_choice] == 1) + 1*([dat(1:end-1).resp] == 2 & [dat(1:end-1).correct_choice] == 0));
