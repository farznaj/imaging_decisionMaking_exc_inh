...
if len(mice) > 1:
    ...
    for im in range(len(mice)):
        ...
        for iday in range(numDays):
            ...
            winhAbsAve_all_allMice_eachFr_aligned[:, iday] = winhAbsAve_all_allMice_eachFr[im][iday][eventI_allDays_allMice[im][iday] - nPreMin  :  eventI_allDays_allMice[im][iday] + nPostMin + 1]
            wexcAbsAve_all_allMice_eachFr_aligned[:, iday] = wexcAbsAve_all_allMice_eachFr[im][iday][eventI_allDays_allMice[im][iday] - nPreMin  :  eventI_allDays_allMice[im][iday] + nPostMin + 1]
    ...
    winhAbsAveDay_aligned = np.array([np.mean(winhAbsAve_all_allMice_eachFr_aligned_all[im], axis=1) for im in range(len(mice))]) # numMouse x numFrames
    wexcAbsAveDay_aligned = np.array([np.mean(wexcAbsAve_all_allMice_eachFr_aligned_all[im], axis=1) for im in range(len(mice))]) # numMouse x numFrames
...
#    poolDaysOfMice = 0 # for plotting w traces; if 0, average and sd across mice (each mouse has session-averaged weights); if 1, average and sd across all sessions of all mice
    ...
    if savefigs:#% Save the figure
        ...
        if poolDaysOfMice==0:
            fign = os.path.join(d, suffn[0:5]+'allFrs_w_absw_aveMice_aveDays_aveNs_excVSinh.'+fmt[0])
        else:
            fign = os.path.join(d, suffn[0:5]+'allFrs_w_absw_aveMiceDays_aveNs_excVSinh.'+fmt[0])    
    ...
    if savefigs:#% Save the figure
        ...
        fign = os.path.join(d, suffn[0:5]+'fractNon0w_allFrs_aveMiceDays_aveNs_excVSinh.'+fmt[0])    
        fig.savefig(fign, bbox_inches='tight')
