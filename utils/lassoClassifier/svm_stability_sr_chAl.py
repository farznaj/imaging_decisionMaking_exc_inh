...
# svm_stability.py includes the codes here... this file includes analysis just for svr, choice-aligned 
# This script should be run on the cluster.


Created on Thu Feb  9 23:55:38 2017
@author: farznaj
"""
...
Ysr_choice = stimrate[~trsExcluded_choice] # we dont need to exclude trials that animal did not make a choice for the stimulus decoder analysis... unless you want to use the same set of trials for the choice and stimulus decoder...
Ysr_choice = (Ysr_choice - (Ysr_choice.min()+np.ptp(Ysr_choice)/2)) / np.ptp(Ysr_choice) # when you did not do this, angle_stim_sh (ie stimulus shuffled) gave you weird results... during baseline (~15 frames at the beginning and almost ~15 frames at the end) you got alignment between population responses in the shuffled data!
...
nrtr = int(np.ceil(.8*len(Y_choice))) # we will subselect 80% of trials


if doData:
    # data
    w_sr_chAl = np.full((np.shape(X_choice)[0], X_choice.shape[1], nRandTrSamp), np.nan) # frames x neurons x nrandtr
    b_sr_chAl =  np.full((X_choice.shape[0], nRandTrSamp), np.nan) # frames x nrandtr
    trainE_sr_chAl =  np.full((X_choice.shape[0], nRandTrSamp), np.nan) # frames x nrandtr
        
    for ir in range(nRandTrSamp): # subselect a group of trials
        print 'BAGGING iteration %d' %(ir)
        
        # subselect nrtr random trials 
        randTrs = rng.permutation(len(Y_choice))[0:nrtr].flatten() # random trials
        
        X = X_choice[:,:,randTrs]
        Y = Ysr_choice[randTrs] # now Y is the vector of stimulus rates
    ...
    #    linear_svm = svm.LinearSVC() # default c, also l2
        ...
        for ifr in range(np.shape(X)[0]):
            ...
            w_sr_chAl[ifr,:, ir] = np.squeeze(svr.coef_);
            b_sr_chAl[ifr, ir] = svr.intercept_;        
            trainE_sr_chAl[ifr, ir] = abs(svr.predict(X[ifr,:,:].transpose())-Y.astype('float')).sum()/len(Y)*100;
...
if doShuffles:
    ...
    # shuffle
    wsh_sr_chAl = np.full((np.shape(X_choice)[0], X_choice.shape[1], nRandTrSamp, nShflSamp), np.nan) # frames x neurons x nrandtr x nshfl
    bsh_sr_chAl =  np.full((X_choice.shape[0], nRandTrSamp, nShflSamp), np.nan) # frames x nrandtr x nshfl
    trainEsh_sr_chAl =  np.full((X_choice.shape[0], nRandTrSamp, nShflSamp), np.nan) # frames x nrandtr x nshfl
    ...
    #                linear_svm = svm.LinearSVC() # default c, also l2
            ...
            for ifr in range(np.shape(X)[0]):
                ...
                wsh_sr_chAl[ifr,:, ir, ish] = np.squeeze(svr.coef_);
                bsh_sr_chAl[ifr, ir, ish] = svr.intercept_;        
                trainEsh_sr_chAl[ifr, ir, ish] = abs(svr.predict(X[ifr,:,:].transpose())-Y[permIxs].astype('float')).sum()/len(Y[permIxs])*100;
...
angle_stim_sh_chAl = np.full((w_sr_chAl.shape[0],w_sr_chAl.shape[0],nShflSamp), np.nan) # frames x frames x nshfl
for ish in range(nShflSamp):
    angle_stim_sh_chAl[:,:,ish] = np.arccos(abs(np.dot(wsh_sr_chAl_n2b[:,:,ish], wsh_sr_chAl_n2b[:,:,ish].transpose())))*180/np.pi # frames x frames x nshfl
...
plt.figure()
plt.imshow(np.nanmean(angle_stim_sh_chAl, axis=2), cmap='jet_r') # average across shuffles
plt.colorbar()
'''
...
if trialHistAnalysis:
    ...
    if doData and doShuffles==0:
        svmn = 'svmCurrChoice_allN_allFrsTrained_srChAl_data_ds%d_%s_' %(regressBins,nowStr)
    elif doData==0 and doShuffles:
        svmn = 'svmCurrChoice_allN_allFrsTrained_srChAl_shfl_ds%d_%s_' %(regressBins,nowStr)
    else:
        svmn = 'svmCurrChoice_allN_allFrsTrained_srChAl_ds%d_%s_' %(regressBins,nowStr)
...
if saveResults:
    ...
    #                           'NsExcluded':NsExcluded, 'trsExcluded':trsExcluded, 'meanX': meanX, 'stdX':stdX, # not sure if you need any of these
                               ...
                               'w_sr_chAl':w_sr_chAl, 'b_sr_chAl':b_sr_chAl, 'trainE_sr_chAl':trainE_sr_chAl,
                               'wsh_sr_chAl':wsh_sr_chAl, 'bsh_sr_chAl':bsh_sr_chAl, 'trainEsh_sr_chAl':trainEsh_sr_chAl})
    
    elif doData:
        scio.savemat(svmName, {'w_sr_chAl':w_sr_chAl, 'b_sr_chAl':b_sr_chAl, 'trainE_sr_chAl':trainE_sr_chAl})


    elif doShuffles:
        scio.savemat(svmName, {'wsh_sr_chAl':wsh_sr_chAl, 'bsh_sr_chAl':bsh_sr_chAl, 'trainEsh_sr_chAl':trainEsh_sr_chAl})
