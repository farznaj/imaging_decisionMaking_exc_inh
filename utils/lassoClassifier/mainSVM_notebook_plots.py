...
print sum(all0d), 'days with all-0 weights: ', np.array(days)[all0days]
...
fign_ = 'projTraces_svm_raw'+suffn
fign = os.path.join(d, fign_+'.'+fmt)
...
#            th_stim_dur = []
        ...
        ##%% Load vars (w, etc)
        Data = scio.loadmat(svmName, variable_names=['w', 'NsExcluded', 'NsRand'])
        w = Data.pop('w')[0,:]    
        NsExcluded = Data.pop('NsExcluded')[0,:].astype('bool')
        NsRand = Data.pop('NsRand')[0,:].astype('bool')
...
fign_ = 'projTraces_excInh_svm_raw'+suffn
fign = os.path.join(d, fign_+'.'+fmt)
