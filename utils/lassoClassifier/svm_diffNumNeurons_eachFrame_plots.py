...
sdCA_allExcSamps_maxN = sdEachExcSamp(CAav_test_exc_alig_sameMaxN0) # max_nNeurs_used_for_training x nAlignedFrs x nExcShfl
...
def plotCA_nN(CA, labs, cols, excInd=-1, sdD=None, doP=0): # excInd is the index of exc in array CAav_alig_sameN... default is -1 so assuming we will get sd across days and not have it as an input
...
pwmt = plotCA_nN(CA, labs, cols, excInd=1, sdD=sdCA_allExcSamps_maxN_ave, doP=1) # for exc use sdCA_allExcSamps_ave as sd instead of taking sd across days (this is more comparable to sd of inh, bc sdCA_allExcSamps_ave is sd across days for each nExcShfl, then averaged across nExcShfl, if we get sd acorr excShfl-averaged traces it will be much lower than inh sd)
